...
describe('simulation success predicates', () => {
	it('account simulation requires on-chain verification', () => {
		...
		expect(isSimulationSuccess(baseResult({ onChainVerified: false }))).toBe(false)
	})
	...
	it('account simulation fails if any WAX check is missing', () => {
		...
		expect(isSimulationSuccess(baseResult({ authorityVerified: false }))).toBe(false)
	})

	it('account simulation fails if broadcasted', () => {
		expect(isSimulationSuccess({ ...baseResult(), broadcasted: true })).toBe(false)
	})

	it('RC simulation allows skipped on-chain verification', () => {
		expect(isRcSimulationSuccess(baseResult({ onChainVerified: false }))).toBe(true)
		expect(isRcSimulationSuccess(baseResult({ signed: false }))).toBe(false)
		expect(isRcSimulationSuccess({ ...baseResult(), broadcasted: true })).toBe(false)
	})
})
