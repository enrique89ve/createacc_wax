...
export type BlockchainErrorCode =
  (typeof BLOCKCHAIN_ERROR_CODES)[keyof typeof BLOCKCHAIN_ERROR_CODES]
...
export const VALIDATION_ERROR_CODES = {
  ...
  USERNAME_TOO_SIMILAR: 'USERNAME_TOO_SIMILAR',
  USERNAME_POLICY_UNAVAILABLE: 'USERNAME_POLICY_UNAVAILABLE',
...
export const BLOCKCHAIN_ERROR_MESSAGES: Record<BlockchainErrorCode, string> = {
  ...
  [BLOCKCHAIN_ERROR_CODES.ACCOUNT_NOT_EXISTS]: 'Account does not exist',
  [BLOCKCHAIN_ERROR_CODES.ACCOUNT_ALREADY_EXISTS]: 'Account already exists',
  [BLOCKCHAIN_ERROR_CODES.INSUFFICIENT_RC]: 'Insufficient resource credits',
  [BLOCKCHAIN_ERROR_CODES.GENERIC_HIVE_ERROR]: 'Hive generic error',
  [BLOCKCHAIN_ERROR_CODES.MISSING_WALLET_CONFIG]:
    'Account and private key are required',
  [BLOCKCHAIN_ERROR_CODES.SELF_DELEGATION]: 'Cannot delegate RC to yourself',
...
export const VALIDATION_ERROR_MESSAGES: Record<ValidationErrorCode, string> = {
  ...
  [VALIDATION_ERROR_CODES.TICKET_RACE_CONDITION]:
    'Ticket operation race condition',
  ...
  [VALIDATION_ERROR_CODES.USERNAME_NOT_ALLOWED]: 'Username not allowed',
  [VALIDATION_ERROR_CODES.USERNAME_TOO_SIMILAR]:
    'Username too similar to a recent account',
  [VALIDATION_ERROR_CODES.USERNAME_POLICY_UNAVAILABLE]:
    'Username policy check unavailable',
  [VALIDATION_ERROR_CODES.INVALID_PUBLIC_KEY_FORMAT]:
    'Invalid public key format',
  [VALIDATION_ERROR_CODES.KEYS_DOWNLOAD_NOT_CONFIRMED]:
    'Keys download not confirmed',
...
export const ERROR_TO_HTTP_STATUS: Record<UnifiedErrorCode, number> = {
  ...
  [VALIDATION_ERROR_CODES.ACCOUNT_CREATION_IN_PROGRESS]: 409,
  [VALIDATION_ERROR_CODES.MISSING_REQUIRED_FIELDS]: 400,
  [VALIDATION_ERROR_CODES.INVALID_USERNAME_FORMAT]: 400,
  [VALIDATION_ERROR_CODES.USERNAME_NOT_ALLOWED]: 400,
  [VALIDATION_ERROR_CODES.USERNAME_TOO_SIMILAR]: 400,
  [VALIDATION_ERROR_CODES.USERNAME_POLICY_UNAVAILABLE]: 503,
