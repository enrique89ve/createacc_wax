...
export const GET: APIRoute = async context => {
	return withAdminSession(context, async session => {
		try {
			...
			try {
				assertCanPerform(session, 'MANAGE_BUILDERS', 'GET /api/management/users')
			} catch {
				return unauthorizedResponse()
			}

			// Obtener todos los builders usando el repository
			const builders = await buildersRepository.getAll()
...
export const POST: APIRoute = async context => {
	return withAdminSession(context, async session => {
		try {
			// RBAC: Check permission
			try {
				assertCanPerform(session, 'CREATE_BUILDER', 'POST /api/management/users')
			} catch {
				return unauthorizedResponse()
			}
			...
			// Verificar que el builder no exista usando el repository
			const exists = await buildersRepository.existsByUsername(cleanUsername)
			...
			// Crear builder usando el repository
			const builderId = await buildersRepository.create(cleanUsername)
...
export const DELETE: APIRoute = async context => {
	return withAdminSession(context, async session => {
		try {
			// RBAC: Check permission
			try {
				assertCanPerform(session, 'DELETE_BUILDER', 'DELETE /api/management/users')
			} catch {
				return unauthorizedResponse()
			}
			...
			// Verificar que el builder existe usando el repository
			const builder = await buildersRepository.getById(Number(userId))
			...
			// Eliminar builder usando el repository
			await buildersRepository.delete(Number(userId))
