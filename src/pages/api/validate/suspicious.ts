...
// Sugerencias para usernames alternativos
const USERNAME_SUGGESTIONS = [
	'Try adding numbers: {username}123',
	'Try adding year: {username}2024', 
	'Try adding location: {username}city',
	'Try adding hobby: {username}code',
	'Try reversing: {reversed}',
	'Try with underscore: {username}_user'
]

/**
 * Genera sugerencias para usernames alternativos
 */
function generateSuggestions(username: string): string[] {
	...
	// Agregar números
	suggestions.push(`${cleanUsername}123`)
	suggestions.push(`${cleanUsername}2024`)
	
	// Si es muy corto, agregar sufijo
	if (cleanUsername.length <= 6) {
		suggestions.push(`${cleanUsername}user`)
		suggestions.push(`${cleanUsername}dev`)
	}
	
	// Si es muy similar a palabra bloqueada, sugerir variación
	if (cleanUsername.includes('admin')) {
		suggestions.push(`${cleanUsername.replace('admin', 'manager')}`)
	}
	if (cleanUsername.includes('test')) {
		suggestions.push(`${cleanUsername.replace('test', 'demo')}`)
	}
	
	// Filtrar sugerencias que también sean sospechosas
	const validSuggestions = suggestions.filter(suggestion => {
		return !isSuspiciousUsername(suggestion) && suggestion.length <= 16
	})
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		// Agregar sugerencias si es sospechoso
		if (isSuspicious) {
			const suggestions = generateSuggestions(username)
			if (suggestions.length > 0) {
				response.suggestion = `Prueba: ${suggestions.join(', ')}`
			}
		}
...
export const GET: APIRoute = async ({ url }) => {
	try {
		...
		if (testUsername) {
			...
			const suggestions = isSuspicious ? generateSuggestions(testUsername) : []
