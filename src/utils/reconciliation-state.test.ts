...
describe('reconciliation state machine', () => {
	it('pending → processing → resolved', async () => {
		...
		expect(await statusOf(id)).toMatchObject({ status: RECONCILIATION_STATUS.RESOLVED })
	...
	it('failed → retry claim', async () => {
		...
		expect(await statusOf(id)).toMatchObject({ status: RECONCILIATION_STATUS.FAILED })
		expect(await claimReconciliationEntry(id, RESOLVER)).toBe(true)
		expect(await statusOf(id)).toMatchObject({
			status: RECONCILIATION_STATUS.PROCESSING,
			attemptCount: 2,
	...
	it('stale processing resets to failed', async () => {
		...
		expect(
			await resetStuckProcessingEntries(RECONCILIATION_CONFIG.PROCESSING_TIMEOUT_MS)
		).toBeGreaterThan(0)
		expect(await statusOf(id)).toMatchObject({ status: RECONCILIATION_STATUS.FAILED })
	})
	...
	it('MAX_ATTEMPTS claim can be abandoned', async () => {
		...
		expect(await claimReconciliationEntry(id, RESOLVER)).toBe(true)
		expect(await markReconciliationAbandoned(id, 'Exceeded MAX_ATTEMPTS')).toBe(true)
		expect(await statusOf(id)).toMatchObject({ status: RECONCILIATION_STATUS.ABANDONED })
		const pending = await getPendingReconciliations()
		expect(pending.some(entry => entry.id === id)).toBe(false)
	})
