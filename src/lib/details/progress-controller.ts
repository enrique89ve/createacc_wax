...
const STEP_CLASSES: Record<StepState, { item: string; icon: string; label: string; status: string }> = {
	pending: {
...
export function updateStepStates(
	...
	steps.forEach(stepEl => {
		...
		const state = resolveStepState(index, currentStep, completedSteps, errorStep)

		stepEl.setAttribute('data-state', state)

		// Update step item classes
		const baseItemClasses = 'step-item flex items-center gap-4 p-4 rounded-lg border transition-all duration-500 ease-out'
		...
		if (iconContainer) {
			const baseIconClasses = 'step-icon flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all duration-300'
