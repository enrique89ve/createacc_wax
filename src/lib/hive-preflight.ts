...
const NO_CLAIMED_ACCOUNTS_MESSAGE = 'Creator account has no pending claimed accounts'

function check(status: PreflightCheckStatus, message: string, detail?: string | number): PreflightCheck {
...
async function checkWaxOperation(
	...
	try {
		...
		const message = error instanceof Error ? error.message : 'WAX validation failed'
		return {
			wax: fail(message),
...
export async function runAccountCreationPreflight(
	...
	try {
		...
		const message = error instanceof Error ? error.message : 'Hive RPC unavailable'
	...
	const passed = blocking.every(item => item.status !== PREFLIGHT_CHECK_STATUS.FAIL)
	...
	if (!passed) {
		const failedCheck = blocking.find(item => item.status === PREFLIGHT_CHECK_STATUS.FAIL)
