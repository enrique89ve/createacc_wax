...
async function runReconciliation(): Promise<void> {
	...
	try {
		...
		logger.info(`[${RESOLVER_ID}] Processing ${mature.length} pending reconciliation(s).`)
...
const interval = setInterval(runReconciliation, RECONCILIATION_CONFIG.AUTO_CHECK_INTERVAL_MS)
interval.unref()

logger.info(`[${RESOLVER_ID}] Started (interval: ${RECONCILIATION_CONFIG.AUTO_CHECK_INTERVAL_MS / 1000}s).`)
