...
describe('uncertain RC recovery', () => {
	...
	it('absent delegation returns to pending for retry', async () => {
		...
		await insertUncertain(username)
		expect(await applyUncertainRcLookup(username, { status: 'not_found' })).toBe('retry')
		expect(await rcRow(username)).toEqual({
	...
	it('mismatch stays uncertain and does not retransmit', async () => {
		...
		await insertUncertain(username)
		expect(
			await applyUncertainRcLookup(username, { status: 'mismatch', delegatedRc: 1n })
		).toBe('hold')
		expect(await rcRow(username)).toEqual({
	...
	it('reconcileUncertainRcDelegations applies the four Hive outcomes', async () => {
		...
		expect(await rcRow(mismatch)).toMatchObject({ rcStatus: RC_STATUS.UNCERTAIN })
		expect(await rcRow(rpcError)).toMatchObject({ rcStatus: RC_STATUS.UNCERTAIN })
		const absentRow = await rcRow(absent)
		expect([RC_STATUS.PENDING, RC_STATUS.PROCESSING]).toContain(absentRow.rcStatus)
	})
})
