...
export default {
	'no-template-literals-in-logging': {
		meta: {
			type: 'security',
			docs: {
				description: 'Previene uso de template literals en logging que pueden causar log injection',
				category: 'Security',
				recommended: true
			},
			schema: [],
			messages: {
				templateLiteralInLog: 'Evita template literals en mensajes de log. Usa mensajes estáticos y coloca datos en metadata.',
				sensitiveFieldInLog: 'Campo sensible "{{field}}" detectado en logging. Usa funciones de sanitización.'
			}
		},
		create(context) {
				...
				CallExpression(node) {
					if (isLoggingCall(node)) {
						...
						for (let i = 0; i < node.arguments.length; i++) {
							...
							if (i >= 2) { // metadata suele ser el 3er argumento
	...
	'require-sanitized-logging': {
		meta: {
			type: 'security',
			docs: {
				description: 'Requiere uso de funciones de sanitización para user input en logs',
				category: 'Security',
				recommended: true
			},
			schema: [],
			messages: {
				requireSanitization: 'User input "{{variable}}" debe ser sanitizado antes de logging. Usa sanitizeLogInput().'
			}
		},
		create(context) {
			// Variables que comúnmente contienen user input
			const userInputVariables = new Set([
				'username',
				'path',
				'error',
				'ip'
