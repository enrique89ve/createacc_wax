...
import { db, initializeDatabase, insertAppUser } from '../src/lib/database'
...
async function seedAdmin(): Promise<string | null> {
	if (await rowExists('user', 'role', 'admin')) {
		console.log('  Admin already exists, skipping.')
		const result = await db.execute(`SELECT id FROM "user" WHERE role = 'admin'`)
...
async function seedBuilder(): Promise<string | null> {
	if (await rowExists('user', 'username', SEED_DEFAULTS.BUILDER_USERNAME)) {
	...
	const builderId = await insertAppUser({
		username: SEED_DEFAULTS.BUILDER_USERNAME,
...
async function seedTicket(createdBy: string): Promise<void> {
